...
I also like to use ~projectile-edit-dir-locals~
...
While performing a search, you can activate the calling action with
~C-o c~ to automatically switch to the result while navigating the
list in the minibuffer.

You can call ivy-occur with ~C-x C-o~ and it will show all the results
in a seperate buffer. While in this buffer
- ~f~ to show the current result in an adjacent buffer
- ~j~ or ~k~ for next or previous result
- ~C-c C-f~ to activate error follow mode. This allows you to navigate
  the error list with emacs error mechanism. ~[e~ and ~]e~ with my
  emacs evil config.
...
Tags are generated through a git hook
...
TODO: fix keys as they don't work because of evil-mode
#+BEGIN_SRC emacs-lisp
