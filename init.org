...
in that directory are automatically added to the Emacs load-path.
#+BEGIN_SRC emacs-lisp
...
I use a special keymap ~my-keys-minor-mode-map~ to store my global keybindings.
...
In visual mode
`R` to mark all
`M-d` to mark next like this
`M-D` mark and previous like this
`C-n` and `C-p` to navigate through marks
`enter` to toggle
#+BEGIN_SRC emacs-lisp
